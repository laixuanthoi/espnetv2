...
class RandomCropResize(object):
    ...
    def __call__(self, img, label):
        ...
        label_crop = cv2.resize(label_crop, self.size,
                                interpolation=cv2.INTER_NEAREST)
...
class RandomFlip(object):
    ...
    def __call__(self, image, label):
        if random.random() < 0.5:
            # random.randint(0, 1) #if you want to do vertical flip, uncomment this line
            x1 = 0
            if x1 == 0:
...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        if self.scale != 1:
            ...
            label = cv2.resize(
                label, (int(w/self.scale), int(h/self.scale)), interpolation=cv2.INTER_NEAREST)

        image = image.transpose((2, 0, 1))

        image_tensor = torch.from_numpy(image).div(255)
        label_tensor = torch.LongTensor(
            np.array(label, dtype=np.int))  # torch.from_numpy(label)
